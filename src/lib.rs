...
fn calculate_constants(word: usize) -> (usize, usize) {
    match word {
        16 => (0xB7E1, 0x9E37),
...
fn align_key(mut key: Vec<u8>, word_bit: usize) -> (usize, Vec<usize>) {
    let table_size: usize;
    let key_len = key.len();

    if key_len == 0 {
        table_size = 1;
    } else if key_len % word_bit != 0 {
        // Increase the size of the key to accommodate the word size being used
        // and fill it with zero and place holders.
        (0..(word_bit - key_len % word_bit)).for_each(|_i| key.push(0x0));
        let key_len = key.len();
        table_size = key_len / word_bit;
    } else {
        table_size = key_len / word_bit;
    }
    ...
    (0..key_len)
        .rev()
        .for_each(|i| table[i / word_bit] = (table[i / word_bit] << shift) + usize::from(key[i]));
    (table_size, table)
}
...
fn extend_key(word: usize, rounds: usize, modulo: usize) -> Vec<usize> {
    let (p_const, q_const) = calculate_constants(word);
    (0..2 * (rounds + 1))
        .map(|index| (p_const + index * q_const) % modulo)
        .collect()
}
...
fn shuffle(
    ...
    round_1x2: usize,
    word: usize,
    mut key_table: Vec<usize>,
    mut aligned_table: Vec<usize>,
) -> Vec<usize> {
    ...
    for _x in 0..(3 * cmp::max(table_size, round_1x2)) {
        key_table[i] = shift_left(key_table[i] + a + b, 3, word);
        a = key_table[i];
        aligned_table[j] = shift_left(aligned_table[j] + a + b, a + b, word);
        b = aligned_table[j];
        i = (i + 1) % round_1x2;
        j = (j + 1) % table_size;
    }
...
fn calculate_mask(word: usize, shift: usize) -> (usize, usize) {
    let base: usize = 2;
...
fn shift_left(val: usize, shift: usize, word: usize) -> usize {
...
/// happen.
fn set_up_key_table(word: usize, rounds: usize, key: Vec<u8>) -> (Vec<usize>, usize) {
    let base: usize = 2;
    let modulo = base.pow(word as u32) as usize;
    let (c, table) = align_key(key, word / 8);
    let key_table: Vec<usize> = extend_key(word, rounds, modulo);
    (shuffle(c, 2 * (rounds + 1), word, key_table, table), modulo)
}

/// `encode` returns a cipher text for a given key and plaintext.
pub fn encode(key: Vec<u8>, plaintext: Vec<u8>, word: usize, rounds: usize) -> Vec<u8> {
    let bits: usize = (word / 8) as usize;
    ...
    let mut a = lsv.read_u32::<LittleEndian>().unwrap() as usize;
    let mut b = rsv.read_u32::<LittleEndian>().unwrap() as usize;
    ...
    (1..rounds + 1).for_each(|round| {
        a = (shift_left(a ^ b, b, word) + key_table[2 * round]) % modulo;
        b = (shift_left(a ^ b, a, word) + key_table[2 * round + 1]) % modulo;
    });
...
pub fn decode(key: Vec<u8>, ciphertext: Vec<u8>, word: usize, rounds: usize) -> Vec<u8> {
    ...
    (1..(rounds + 1)).rev().for_each(|round| {
        b = b
            .overflow_sub(key_table[2 * round + 1] as u32)
            .rotate_right(a)
            ^ a;
        a = a
            .overflow_sub(key_table[2 * round] as u32)
